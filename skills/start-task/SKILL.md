    ...
    <phase id="2" name="Memory Update & Planning">
      <step id="2.1">
        <action>
          ...
          Drafting the plan (only if no plan exists yet): You MUST use the `write_to_file` tool to draft the step-by-step implementation plan directly inside the `<implementation_plan>` block of the `task_[name].md` file ON DISK. Use `<step id="N" status="pending">[Description]</step>` format.
          VERIFICATION LOCK: Before asking for user approval, you MUST verify the file `task_[name].md` was successfully created on the filesystem. Before responding to the user, you MUST explicitly state in your response that the file was successfully written to disk. Do not present the plan if the file does not exist.
    ...
    <phase id="3" name="Execution (Iterative TDD)">
      ...
      <step id="3.2">
        <action>
          ...
          You MUST use the `replace_file_content` tool to physically update the step's status to 'complete' in the tracking file.
