...
| `SCHEMA_REFERENCE.md` | Auto-generated by sync-schema-docs. Maps DB tables to business logic in `SPEC.md`. |
...
| Key | Purpose |
|-----|---------|
| Schema path | Path to the raw database schema (e.g. `db/schema.rb`, `prisma/schema.prisma`, `db/schema.ts`). If not specified, sync-schema-docs infers from common paths. |
